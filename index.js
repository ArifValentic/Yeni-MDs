...
async function startstyle() {
    ...
    style.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let pa7rick = await style.sendContact(callerId, global.owner)
    style.sendMessage(callerId, { text: `Sistem otomatis block!\nJangan menelpon bot!\nSilahkan Hubungi Owner Untuk Dibuka !`}, { quoted : pa7rick })
    await sleep(8000)
    ...
    style.ev.on('messages.upsert', async chatUpdate => {
        ...
        m = smsg(style, mek, store)
        require("./style")(style, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    style.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
    ...
    style.ev.on('contacts.update', update => {
        for (let contact of update) {
            let id = style.decodeJid(contact.id)
            if (store && store.contacts) store.contacts[id] = { id, name: contact.notify }
        }
    })

    style.getName = (jid, withoutContact  = false) => {
        id = style.decodeJid(jid)
        withoutContact = style.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = style.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? {
            id,
            name: 'WhatsApp'
        } : id === style.decodeJid(style.user.id) ?
            style.user :
            (store.contacts[id] || {})
            return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    }
    
    style.sendContact = async (jid, kon, quoted = '', opts = {}) => {
	let list = []
	for (let i of kon) {
	    list.push({
	    	displayName: await style.getName(i + '@s.whatsapp.net'),
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await style.getName(i + '@s.whatsapp.net')}\nFN:${await style.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:okeae2410@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/cak_haho\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	style.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
    }
    ...
    style.serializeM = (m) => smsg(style, m, store)
    ...
    style.ev.on('connection.update', async (update) => {
        ...
        let reason = lastDisconnect.error ? lastDisconnect?.error?.output.statusCode : 0;
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startstyle(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startstyle(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); process.exit(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Delete Session and Scan Again.`); process.exit(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startstyle(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startstyle(); }
     ...
     * @returns
     */
    style.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: style.waUploadToServer })
        ...
        imageMessage: message.imageMessage,
            ...
            style.relayMessage(jid, template.message, { messageId: template.key.id })
    }
     ...
     * @param {*} options 
     */
    style.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            text,
            footer,
            buttons,
        ...
        style.sendMessage(jid, buttonMessage, { quoted, ...options })
    }
     ...
     * @returns 
     */
    style.sendVideo = async (jid, path, caption = '', quoted = '', gif = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
...
const sendFileFromUrl = async (from, url, caption, m, men) => {
    ...
    style.downloadMediaMessage = async (message) => {
        let mime = (message.msg || message).mimetype || ''
        let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
     ...
     * @returns 
     */
    style.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
        let types = await style.getFile(path, true)
       ...
       await style.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options })
    ...
    style.cMod = (jid, copy, text = '', sender = style.user.id, options = {}) => {
		...
		copy.key.fromMe = sender === style.user.id
